...
Chart.register({
  id:'zeroLine',
  afterDraw(chart){
    if(!chart.options.indexAxis||chart.options.indexAxis!=='y') return;
    ...
    ctx.font='bold 9px -apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif';
    ctx.fillStyle='#999';ctx.textAlign='center';ctx.fillText('0%',xPixel,chart.chartArea.bottom+12);
    ctx.restore();
  }
});
